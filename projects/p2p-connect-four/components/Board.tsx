...
const Board: React.FC<BoardProps> = ({
  ...
  useEffect(() => {
    if (winner && winner !== 'DRAW') {
      ...
      if (winner === myPlayer) {
         confetti({ particleCount: 150, spread: 70, origin: { y: 0.6 }, colors: ['#f43f5e', '#fbbf24'] });
      }
    } else {
  ...
  const getDiscStyles = (value: number, r: number, c: number) => {
    ...
    let base = "w-full h-full rounded-full transition-all duration-300 transform ";
    
    if (value === Player.One) {
        base += "bg-gradient-to-br from-rose-500 to-rose-700 shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_8px_rgba(0,0,0,0.3)]";
    } else if (value === Player.Two) {
        base += "bg-gradient-to-br from-amber-400 to-amber-600 shadow-[inset_0_2px_4px_rgba(255,255,255,0.5),0_4px_8px_rgba(0,0,0,0.3)]";
    } else {
      ...
      <div className="glass-ios rounded-[32px] p-2 sm:p-4 relative z-10 border border-white/5 shadow-2xl">
        ...
        <div className="absolute -top-6 sm:-top-10 left-0 right-0 h-6 sm:h-10 flex px-2 sm:px-4 pointer-events-none">
          {Array.from({ length: COLS }).map((_, c) => (
             <div key={`ghost-${c}`} className="flex-1 flex justify-center items-end px-0.5 sm:px-1">
                {hoverCol === c && canPlay && !winner && (
                   <div className={`w-[80%] aspect-square rounded-full opacity-60 mb-1 transition-all duration-200 ${
                      myPlayer === Player.One ? 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.6)]' : 'bg-amber-400 shadow-[0_0_10px_rgba(251,191,36,0.6)]'
                   }`} />
                )}
             </div>
          ))}
        ...
        {/* Grid Container */}
        <div className="grid grid-cols-7 gap-1.5 sm:gap-3 bg-white/5 rounded-[20px] p-1.5 sm:p-3 border border-white/5 shadow-inner"
          ...
          {Array.from({ length: COLS }).map((_, col) => (
            ...
            >
              {board.map((row, r) => (
                // Responsive Size: 11vw for mobile, capped at 64px for desktop
                <div key={`${r}-${col}`} className="w-[11vw] h-[11vw] max-w-[50px] max-h-[50px] sm:max-w-[64px] sm:max-h-[64px] relative">
                   {/* Hole Background */}
